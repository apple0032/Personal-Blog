...
router.get('/', function(req, res, next) {
    var query = 'SELECT * FROM article ORDER BY articleID DESC LIMIT 3';
    mysql.query(query, function(err, rows, fields){
       ...
       res.render("index", {articles: articles,user:req.session.user , title: title});
    });
});

router.post('/index', function(req, res, next) {
    var email = req.body.email;
    var query = 'INSERT subscribe SET email=' + mysql.escape(email) + ',articleTime=CURDATE()';
    mysql.query(query, function(err, rows, fields) {
        if(err) {
            console.log(err);
            return;
...
router.get('/login', function(req, res, next) {
  ...
  res.render('login', {message:'',user:req.session.user, title:title});
});
   ...
   mysql.query(query, function(err, rows, fields) {
      ...
      mysql.query(query, function(err, rows, fields) {
         if(err) {
          ...
          res.render('article', {article:article,user:req.session.user, title: title});
      });
   });
});
router.get('/edit', function(req, res, next) {
   ...
   res.render('edit',{user:req.session.user, title:title});
});
...
router.get('/friends', function(req, res, next){
    ...
    res.render('friends', {user:req.session.user, title: title});
});
router.get('/about', function(req, res, next) {
    var title= 'about';

   res.render('about', {user:req.session.user,title:title});
});
...
router.get('/stock', function(req, res, next) {
    ...
    res.render('stock', {user:req.session.user, title:title});
});
